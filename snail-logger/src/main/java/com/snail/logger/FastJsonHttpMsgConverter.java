...
import com.snail.common.util.StringUtils;
...
public class FastJsonHttpMsgConverter extends AbstractHttpMessageConverter<Object> {
    ...
    protected PropertyPreFilter[] parseFilter(RespDto respDto) {
        List<PropertyPreFilter> filters = new ArrayList<>();
        if (respDto.getIncludes() != null)
            for (Map.Entry<Class<?>, Set<String>> classSetEntry : respDto.getIncludes().entrySet()) {
        ...
        if (respDto.getExcludes() != null)
            for (Map.Entry<Class<?>, Set<String>> classSetEntry : respDto.getExcludes().entrySet()) {
                SimplePropertyPreFilter filter = new SimplePropertyPreFilter(classSetEntry.getKey());
